    ...
    (lambda ()
      (if (procedure? proc)
          (let ((args (procedure-arguments proc)))
            (format #f "(~a ~a)" 
                    proc-name 
                    (string-join (map symbol->string (or (assoc-ref args 'required) '())) " ")))
          proc-name))
    (lambda _ proc-name)))
  ...
  (catch #t
    (lambda ()
      (let* ((str-list (map symbol->string mod-name))
             ...
             (full-path (search-path %load-path (string-append rel-path ".scm"))))
        full-path))
    (lambda _ #f)))

(define (scan-imports code-str)
  (let ((port (open-input-string code-str)) (modules '()))
    (catch #t
      (lambda ()
        (let loop ()
          (let ((expr (read port)))
            (unless (eof-object? expr)
              (match expr
                (('use-modules args ...) (set! modules (append modules (map cleanup-spec args))))
                (('define-module name . rest)
                 (let scan-opts ((opts rest))
                   (match opts
                     ((#:use-module mod . rem) (set! modules (cons (cleanup-spec mod) modules)) (scan-opts rem))
                     ((kw val . rem) (scan-opts rem))
                     (_ #f))))
                (_ #f))
              (loop)))))
      (lambda _ #f))
...
(define (cleanup-spec spec)
  (if (and (pair? spec) (pair? (car spec))) (car spec) spec))
  ...
  (cond
   ((procedure? obj)
    (let ((src (procedure-source obj)))
      (if (and src (string? (source-property src 'filename)))
          ;; Interpreted/Source Property
          (let ((full-path (resolve-path (source-property src 'filename))))
            (if (and full-path (file-exists? full-path))
                `((file . ,full-path)
                  (line . ,(source-property src 'line))
                  (column . ,(source-property src 'column)))
                #f))
          ;; VM Programs
          (if (program? obj)
              (let ((sources (program-sources obj)))
                (if (and (pair? sources) (pair? (car sources)))
                    (let* ((entry (car sources))
                           (props (cdr entry))
                           (raw-file (assoc-ref props 'filename)))
                      (if (and raw-file (string? raw-file))
                          (let ((full-path (resolve-path raw-file)))
                            (if (and full-path (file-exists? full-path))
                                `((file . ,full-path) 
                                  (line . ,(or (assoc-ref props 'line) 0)) 
                                  (column . ,(or (assoc-ref props 'column) 0)))
                                #f))
                          #f))
                    #f))
              #f))))
                      ...
                      (let ((loc (get-location-for-obj (variable-ref var))))
                        (if loc 
                            ...
                            (let ((mod-file (module-name->path mod-name)))
                              (if mod-file
                                  (begin
                                    (log-msg (format #f "Fallback: Opening module file ~a" mod-file))
                                    `((file . ,mod-file) (line . 0) (column . 0)))
                                  (loop (cdr mods))))))
                      (loop (cdr mods)))))
              (lambda _ (loop (cdr mods)))))))))
...
(define (get-definition-location symbol-name code-str context-list)
  (let ((sym (string->symbol symbol-name)) 
    ...
    (or
     ;; Context Guessing
     (and (list? context-list) (not (null? context-list))
          (let loop ((ctx context-list))
            (if (null? ctx) #f
                (let ((loc (try-getter-location (car ctx) symbol-name imports)))
                  (if loc loc (loop (cdr ctx)))))))

     ;; Direct Lookup
     (let loop ((mods imports))
       (if (null? mods)
           (get-location-for-symbol sym (current-module))
           (let ((mod-name (car mods)))
             (catch #t
               (lambda ()
                 (let* ((mod (resolve-interface mod-name))
...
(define (get-location-for-symbol sym mod)
  (let ((var (module-variable mod sym)))
    (if (and (variable? var) (variable-bound? var))
        (get-location-for-obj (variable-ref var))
        #f)))
               ...
               (lambda ()
                 (let* ((mod (resolve-interface mod-name))
                   ...
                   (let ((sig (format-signature val getter-name))
                     ...
                     (format #f "**~a**\n\n~a\n\n(Defined in ~a)" sig (or doc "Field Accessor") mod-name))))
               (lambda _ (loop (cdr mods)))))))
     (catch #t
       (lambda ()
         (let ((val (module-ref (current-module) getter-sym)))
           (let ((sig (format-signature val getter-name))
                 (doc (object-documentation val)))
             (format #f "**~a**\n\n~a" sig (or doc "Field Accessor")))))
       (lambda _ #f)))))

(define (get-docs-with-context symbol-name code-str context-list)
  (let ((sym (string->symbol symbol-name)) (imports (scan-imports code-str)))
    (or 
     (and (list? context-list) (not (null? context-list))
          (let loop ((ctx context-list))
            (if (null? ctx) #f (let ((doc (try-getter-lookup (car ctx) symbol-name imports))) (if doc doc (loop (cdr ctx)))))))
     (let loop ((mods imports))
       (if (null? mods) 
           (catch #t 
             (lambda () (let ((val (module-ref (current-module) sym)))
                          (format #f "**~a**\n\n~a" (format-signature val symbol-name) (object-documentation val))))
             (lambda _ #f))
           (let ((mod-name (car mods)))
             (catch #t
               (lambda ()
                 (let* ((mod (resolve-interface mod-name)) (val (module-ref mod sym)))
                   (format #f "**~a**\n\n~a\n\n(Found in ~a)" (format-signature val symbol-name) (or (object-documentation val) "No docstring.") mod-name)))
               (lambda _ (loop (cdr mods)))))))
     "No documentation found.")))

(define (get-indent-info symbol-name) (if (or (string-prefix? "def" symbol-name) (string-prefix? "with-" symbol-name)) '((style . "body")) '((style . "align"))))
(define (get-completions prefix) '()) 
(define (eval-code code-str module-str) "Eval not implemented")
(define (make-response id result) `((jsonrpc . "2.0") (id . ,id) (result . ,result)))
